...
# 模型，数据，损失 需要放到gpu
device = torch.device('cpu')  # device = torch.device('cuda:0')  'cuda:1'
...
model = Model()
model = model.to(device)  # 不赋值也可以

learning_rate = 0.01 # 1e-2
loss_func = torch.nn.CrossEntropyLoss()
optimizer = torch.optim.SGD(model.parameters(), lr=learning_rate)

loss_func = loss_func.to(device)  # 不赋值也可以
...
for i in range(epoch):
    ...
    for data in train_loader:
        ...
        if train_step % 100 == 0:
            end_time = time.time()
            print(f'训练次数：{train_step}, loss：{loss.item()}, 时间：{end_time - start_time}')
